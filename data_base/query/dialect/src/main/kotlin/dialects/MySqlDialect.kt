package gog.my_project.data_base.query.dialect.dialects
...
class MySqlDialect(
    ...
    override fun getColumnBaseSql(columnData: IQueryToolsColumnsBase?): String? {
        if (columnData != null){
            val table = columnData.getTable();
            val tableColumn = columnData.getColumn();

            val cte = columnData.getCte();
            val cteSelect = columnData.getSelect();
    ...
    override fun getTableSql(tableData: IQueryToolsTable?, withPrefix: Boolean) : String{
        if (tableData != null){
            val table: QBTable? = tableData.getTable();
            val cte:   QBCte? = tableData.getCte();
